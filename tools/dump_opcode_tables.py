...
def find_module_base(handle, module_name):
    ...
    Использует CreateToolhelp32Snapshot — работает надёжно
    даже для 32-bit процессов из 64-bit Python (в отличие от EnumProcessModulesEx).
    """
    # Сначала пробуем CreateToolhelp32Snapshot (не требует handle)
    pid = find_module_base._current_pid  # set by caller
    base, size = _find_module_toolhelp(pid, module_name)
    if base is not None:
        return base, size

    # Fallback: EnumProcessModulesEx
...
# PID хранится для передачи в find_module_base (toolhelp не требует handle)
find_module_base._current_pid = 0
...
def _find_module_toolhelp(pid, module_name):
    """Найти модуль через CreateToolhelp32Snapshot (надёжно для wow64)."""
    snap = kernel32.CreateToolhelp32Snapshot(
        TH32CS_SNAPMODULE | TH32CS_SNAPMODULE32, pid)
    if snap == ctypes.c_void_p(-1).value or snap == -1:
        err = ctypes.get_last_error()
        print(f"  CreateToolhelp32Snapshot failed: error {err}", file=sys.stderr)
        return None, None
    ...
    if Module32FirstW(snap, ctypes.byref(me)):
        while True:
            if me.szModule.lower() == module_name.lower():
                found_base = me.modBaseAddr
                found_size = me.modBaseSize
                break
            if not Module32NextW(snap, ctypes.byref(me)):
                break

    kernel32.CloseHandle(snap)
    return found_base, found_size
...
def main():
    ...
    # Сначала попробуем найти Core.dll через Toolhelp (не требует handle)
    find_module_base._current_pid = pid
    toolhelp_base, toolhelp_size = _find_module_toolhelp(pid, "Core.dll")
    if toolhelp_base:
        print(f"Core.dll found via Toolhelp: base=0x{toolhelp_base:08X}, size=0x{toolhelp_size:08X}")

    # Открыть процесс для ReadProcessMemory
